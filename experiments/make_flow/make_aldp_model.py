...
def make_aldp_model(config, device):
    ...
    dih_ind_ = (
        target.coordinate_transform.transform.ic_transform.dih_indices.cpu().numpy()
    )
    ...
    circ_shift = (
        None if not "circ_shift" in config["flow"] else config["flow"]["circ_shift"]
    )

    # Base distribution
    if config["flow"]["base"]["type"] == "gauss":
        base = nf.distributions.DiagGaussian(
            ndim, trainable=config["flow"]["base"]["learn_mean_var"]
        )
        ...
        if flow_type == "rnvp":
            ...
            layers.append(
                nf.flows.AffineCouplingBlock(
                    param_map, scale=scale, scale_map=scale_map
                )
            )
            ...
            raise NotImplementedError(
                "The flow type " + flow_type + " is not implemented."
            )
    ...
    if "replay_buffer" in config["training"]:
        min_is_target = (
            min_is_target
            or config["training"]["replay_buffer"]["type"] == "prioritised"
        )
